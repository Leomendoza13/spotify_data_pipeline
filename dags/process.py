...
def process_data():
    ...
    df_top_tracks, df_albums, df_artists, df_available_markets, df_playlists = (
        pd.DataFrame(columns=columns) for columns in [
            ["track_id", "track_name", "added_at", "album_id", "artist_id", "popularity", "preview_url", "is_explicit", "position"],
            ["album_id", "album_name", "release_date", "total_tracks", "album_type", "album_image_url"],
        ...
        if blob.name.endswith('.json'):
            ...
            for position, item in enumerate(data["items"], start=1):
                ...
                df_top_tracks = pd.concat([
                    df_top_tracks,
                    pd.DataFrame([{
                        "track_id": track["id"], "track_name": track['name'], "added_at": item["added_at"],
                        "album_id": album["id"], "artist_id": track["artists"][0]["id"],
                        "popularity": track.get("popularity", 0), "preview_url": track.get("preview_url"),
                        "is_explicit": track.get("explicit", False), "position": position
                    }])
                ], ignore_index=True)
                # Populate `df_albums`
                df_albums = pd.concat([
                    ...
                    pd.DataFrame([{
                        ...
                        "release_date": album.get("release_date"), "total_tracks": album.get("total_tracks"),
                ...
                for artist in track["artists"]:
                    df_artists = pd.concat([
                        ...
                        pd.DataFrame([{
                            ...
                            "artist_url": artist["external_urls"]["spotify"], "genres": ""
                        }])
                    ], ignore_index=True)
                ...
                for market in track.get("available_markets", []):
                    df_available_markets = pd.concat([
                        ...
                        pd.DataFrame([{
                            "track_id": track["id"], "market": market
                        }])
                    ], ignore_index=True)
                ...
                df_playlists = pd.concat([
                    df_playlists,
                    pd.DataFrame([{
                        "playlist_id": "example_playlist_id", "playlist_name": "Top 50",
                        "playlist_description": "Top 50 des chansons", "playlist_owner": "spotify"
                    }])
                ], ignore_index=True)
        break
        ##
